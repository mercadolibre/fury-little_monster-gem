...
describe LittleMonster::RSpec::Matchers::HaveCallbackRetries do
  ...
  describe '#matches?' do
    context 'given a job_class' do
      it 'returns true if job has the expected retries' do
        job_class = double(callback_max_retries: expected_retries, class: Class)
        expect(subject.matches? job_class).to be true
      end

      it 'returns false if job does not have the expected retries' do
        job_class = double(callback_max_retries: expected_retries+1, class: Class)
  ...
  describe 'failure_message' do
    ...
    specify do
      expect(subject.failure_message).to eq("expected job to have callback retries #{expected_retries} " \
  ...
  describe 'failure_message_when_negated' do
    specify do
      expect(subject.failure_message_when_negated).to eq("expected job not to have callback retries #{expected_retries} " \
                                                         "but it does")
    end
  end
end
